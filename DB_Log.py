...
def log_reg(conn, username, password, mail):
    sql = "INSERT INTO t_logins (username, password, mail) VALUES (?,?,?)"
    cursor = conn.cursor()
...
def client_up(conn, ref, nom, prenom, adresse, ville, tel, email, tva, id):
    sql = "UPDATE clients SET ref = ?, nom = ?, prenom = ?, adresse = ?, ville = ?, tel = ?, email = ?, tva = ? WHERE id = ?"
    cursor = conn.cursor()
    cursor.execute(sql, (ref, nom, prenom, adresse, ville, tel, email, tva, id))
    conn.commit()

def client_search(conn, ref, nom, prenom, adresse, ville, tel, email, tva):
    sql = "SELECT * FROM clients WHERE ref = ? OR nom = ? OR prenom = ? OR adresse = ? OR ville = ? OR tel = ? OR email = ? OR  tva = ?"
    cursor = conn.cursor()
    cursor.execute(sql, (ref, nom, prenom, adresse, ville, tel, email, tva))
    rows = cursor.fetchall()
    conn.close()
